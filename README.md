...
- **3D 레이싱** (`3D 레이싱 v1` ~ `v8`): F1 서킷 8개. 아케이드/리얼리스틱 규칙(타이어·피트·날씨·데미지·예선·깃발·세이프티카·ERS), AI 성격, 리플레이·TV 카메라, 트랙 공유 코드, 엔진 사운드(v7). 레벨·도전과제·튜닝, 연습 모드, 서킷별 기록과 고스트, 실시간 변수 랭킹·저장, 주변 차 소리(v8). 처음이라면 `3D 레이싱 소개.md`(사용법과 사용된 기술), 버전별 변경은 `3D 레이싱 v7 설명서.md`
- **3D 포탈 퍼즐**
- **DOOM (Freedoom E1M1)**
