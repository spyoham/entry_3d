// ============================================================
// render.js - the perspective renderer, built on Entry's pen-fill primitive.
//
//   per frame:  cull object -> one 3x3 matrix per object -> transform its mesh
//   vertices once -> per face: light, fog, near-plane clip (Sutherland-Hodgman),
//   back-face cull by signed screen area -> bucket the polygon by depth ->
//   fill back to front (painter's algorithm).
// ============================================================
...
let maxFaces = 460;
let lodSize = 44;        // switch to the simple mesh below this pixel size
let wireHex = '#12161c';

let lightX = -0.42; let lightY = 0.80; let lightZ = -0.42;
let lightI = 0.62; let ambI = 0.45;

let nSv = 0; let nPg = 0; let nDraw = 0; let drawn = 0;
let polyOk = 0; let polyStart = 0; let polyN = 0; let polyFlip = 0;
let layerBias = 0;
let clipOn = 1;          // 0 when the whole object sits in front of the near plane
let pushIdx = 0;
...
function setupCam() {
    ...
    let cr = cosd(camRoll);
    let sr = sind(camRoll);
    crX = sxx * cr + ux * sr; crY = uy * sr; crZ = szz * cr + uz * sr;
    cuX = ux * cr - sxx * sr; cuY = uy * cr; cuZ = uz * cr - szz * sr;
...
// is in the way the camera slides in front of it, so the view never ends up
// inside geometry.
...
function setQuality(q) { maxFaces = q; if (maxFaces < 10) { maxFaces = 10; } if (maxFaces > MAXDR - 10) { maxFaces = MAXDR - 10; } }
...
// Culling happens before any colour work: most faces die here.
function addPoly(a, b, c, d, two) {
    polyOk = 0;
    let need = 3;
    if (d > 0) { need = 4; }
    // objects that cannot reach the near plane skip the test entirely
    let nIn = need;
    if (clipOn == 1) {
        nIn = 0;
...
function buildMat(id) {
    ...
    if (oBill[id] == 1) { ry = camYaw; rx = 0; rz = 0; }
...
function drawObject(id, m) {
    ...
    if (nSv + vn + 6 <= MAXSV) {
        ...
        nSv = base + vn;
        // faces
        let r0 = oR[id]; let g0 = oG[id]; let b0 = oB[id];
        let lit = 1;
        if (oUnlit[id] == 1) { lit = 0; }
        let fs = mFS[m];
        let fn = mFN[m];
        let f = 1;
        while (f <= fn) {
            ...
            if (polyOk == 1) {
                ...
                let fc = fpCol[q];
                if (fc >= 0) {
                    rr = Math.floor(fc / 65536);
                    gg = Math.floor(fc / 256) - rr * 256;
    ...
    if (gap > -0.6) {
        if (gap < 7) {
            ...
            if (depth > NEARZ) {
                if (depth < shadowFar) {
                    if (nSv + 4 <= MAXSV) {
                        // the four corners, transformed straight into the buffer
...
function drawSky() {
    ...
    // sky bands from the horizon colour up to the zenith colour
    let b = 0;
    while (b < 7) {
        let o0 = b * 34;
        let o1 = o0 + 35;
                        ...
                        if (cx > 0 - lx) {
                            ...
                            if (cy < ly) {
                                if (cy > 0 - ly) {
                                    ...
                                    let mm = oMesh[id];
                                    // far away? draw the cheap version of the mesh
                                    if (mLod[mm] > 0) { if (r * camScale < cz * lodSize) { mm = mLod[mm]; } }
                                    drawObject(id, mm);
                                    if (shadowOn == 1) {
                                        if (oShadow[id] == 1) { clipOn = 1; layerBias = 0; addShadow(id); }
                                    }
                                }
                            }
                        }
