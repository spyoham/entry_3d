...
export function compileProgram(sources, { consts: extConsts = {}, funcWeights = {}, labels = {} } = {}) {
    ...
    // Entry's number block hands its text to calc_basic; '+' treats text that
    // fails /^-?\d+\.?\d*$/ (e.g. "1e-30") as a string and concatenates. Emit
    // plain decimals only.
    const plain = (v) => {
        let s = String(v);
        if (!/e/i.test(s)) return s;
        if (Math.abs(v) >= 1) return BigInt(Math.round(v)).toString();
        s = v.toFixed(40).replace(/0+$/, '');
        return s.endsWith('.') ? s + '0' : s;
    };
    const num = (v) => B('number', [plain(v)]);
    ...
    function callExpr(node, scope, pre, asValue) {
        ...
        switch (name) {
            ...
            case 'sq': return B('calc_operation', [null, A(0), null, 'square']);
            ...
            case 'appendText': return B('text_append', [A(0), null]);
            case 'hideAnswer': return B('set_visible_answer', ['HIDE', null]);
            case 'say': return B('dialog', [A(0), 'speak', null]);
        }
    ...
    for (const f of funcs.values()) {
        ...
        // parameter chain. `labels[name]` is a signature such as
        //   '상자 만들기  x %1  y %2  색 %3'   (%n = the n-th parameter)
        // and becomes label/param fields in that order, so the block reads
        // like a sentence in the editor. Without one the fields are just
        // `name (p1) (p2) ...`.
        const sig = labels[f.name];
        const parts = [];
        if (sig) {
            const seen = new Set();
            for (const tok of sig.split(/(%\d+)/)) {
                if (/^%\d+$/.test(tok)) {
                    const i = Number(tok.slice(1)) - 1;
                    if (i < 0 || i >= f.params.length) throw new Error(`label ${f.name}: %${i + 1} out of range`);
                    if (seen.has(i)) throw new Error(`label ${f.name}: %${i + 1} used twice`);
                    // Entry maps a call's arguments to the definition's field
                    // chain positionally, so the chain must follow declaration order
                    if (i !== seen.size) throw new Error(`label ${f.name}: %${i + 1} out of order`);
                    seen.add(i); parts.push({ p: i });
                } else if (tok.trim()) parts.push({ t: tok.trim() });
            }
            if (seen.size !== f.params.length) throw new Error(`label ${f.name}: ${f.params.length} params, ${seen.size} used`);
            if (!parts.length || parts[0].p !== undefined) parts.unshift({ t: f.name });
        } else {
            parts.push({ t: f.name });
            for (let i = 0; i < f.params.length; i++) parts.push({ p: i });
        }
        let chain = null;
        for (let i = parts.length - 1; i >= 1; i--) {
            chain = parts[i].p !== undefined
                ? { ...B('function_field_string', [B(`stringParam_${f.paramIds[parts[i].p]}`, []), chain]), copyable: false, assemble: false }
                : { ...B('function_field_label', [parts[i].t, chain]), copyable: false };
        }
        const label = { ...B('function_field_label', [parts[0].t, chain]), copyable: false };
        ...
        outFunctions.push({
            jsName: f.name, order: outFunctions.length,
