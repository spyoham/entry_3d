...
if (opt('count', null)) {
    await page.evaluate(() => {
        ...
        window.__f0 = Number(Entry.variableContainer.variables_.find(v => v.name_ === 'r_frame').value_);
    });
    await page.waitForTimeout(Number(opt('count')));
    const res = await page.evaluate(() => ({ frames: Number(Entry.variableContainer.variables_.find(v => v.name_ === 'r_frame').value_) - window.__f0, cnt: [...window.__cnt.entries()], typ: [...window.__typ.entries()] }));
