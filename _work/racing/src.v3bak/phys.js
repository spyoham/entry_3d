...
function carPhys(c) {
    ...
    else if (surf >= 2) { gripMul = 0.52; rollRes = 0.30; topMul = 0.55; }
    ...
    if (aF > SLIPPK) { gripF = gripF * (1 - 0.34 * Math.min(1, (aF - SLIPPK) / 14)); }
    let aR = Math.abs(slipR);
    if (aR > SLIPPK) { gripR = gripR * (1 - 0.22 * Math.min(1, (aR - SLIPPK) / 14)); }
    ...
    if (caAir[c] == 0) {
        ...
        if (caSurf[c] >= 2) { if (sp > 7) { slipping = 1; } }
    }
    if (slipping > 0) {
        if (mod(frameId, 2) < 1) {
