...
// ============================================================
export const C = {
    // Entry refuses a list longer than 5000 items and the vertex buffers are
    // (NSEG + 1) * PPR long, so NSEG must stay under about 620.
    NSEG: 400,          // centreline rings per circuit (ring NSEG+1 == ring 1)
    PPR: 8,             // points per ring in the vertex buffer
    NCAR: 8,            // cars on track (1 player + 7 AI)
    NMAT: 176,          // materials in the palette
    NFOG: 16,           // fog levels baked per material
    NTRK: 3,            // built-in circuits
    ...
    NSMOKE: 48,         // tyre-smoke particles
    MKS: 3,             // tyre-mark slots per segment
    NCARV: 24,          // vertices in the car model
    NCARF: 16,          // faces in the car model
    NSCENE: 760,        // scenery instances placed around a circuit
    NSCNV: 40,          // vertices in the largest scenery model
...
export const P = { L: 1, R: 2, GL: 3, GR: 4, WL: 5, WR: 6, CL: 7, CR: 8 };
...
const BASE = {
    road: [72, 74, 82], grass: [64, 132, 62], curbA: [206, 58, 54], curbB: [232, 232, 236],
    wall: [150, 152, 162], tunnel: [96, 94, 104],
    dirt: [150, 128, 88], dark: [34, 44, 40],
};
const CARCOL = [
...
export function buildPalette() {
    ...
    idx.sand = push('sand', [198, 180, 134]);
...
export const TRACKS = [
    {
        // fast permanent circuit: a 700 m pit straight, two long sweepers,
        // a stadium complex and a 44 m hairpin onto the back straight
        name: 'SUNSET RING', sky: [244, 156, 96], fogFar: 520, laps: 3,
        pts: circuit({
            segs: [
            ...
            flag: (u) => (u > 0.452 && u < 0.478 ? 2 : 0),       // jump over the crest
        }),
    },
    {
        // mountain road: narrow, steep, hairpin pairs, a rock tunnel and a
        // blind crest that throws the car into the air
        name: 'CANYON RALLY', sky: [126, 170, 216], fogFar: 420, laps: 3,
        pts: circuit({
            segs: [
            ...
            hill: (u) => 32 * Math.sin(u * 6.2832 + 0.3) + 14 * Math.sin(u * 6.2832 * 3 + 0.9)
                + 6 * Math.sin(u * 6.2832 * 5 + 2.2),
            width: (u) => 8.6 + 1.1 * Math.sin(u * 6.2832 * 4),
            flag: (u) => (u > 0.585 && u < 0.695 ? 1 : 0) | (u > 0.268 && u < 0.296 ? 2 : 0),
        }),
    },
    {
        // night street circuit: walled all the way round, two very long
        // straights and hard 90 degree junctions with a tight chicane
        name: 'HARBOUR NIGHT', sky: [26, 32, 58], fogFar: 380, laps: 3,
        pts: circuit({
            segs: [
...
export function buildData() {
    ...
    off.push(cx.length); cnt.push(0);                 // slot 4 = editor track
    ...
    lists.trkFar = TRACKS.map(t => t.fogFar).concat([420]);
    lists.trkName = TRACKS.map(t => t.name).concat(['MY CIRCUIT']);
    lists.trkSkyR = TRACKS.map(t => t.sky[0]).concat([TRACKS[0].sky[0]]);
    lists.trkSkyG = TRACKS.map(t => t.sky[1]).concat([TRACKS[0].sky[1]]);
    lists.trkSkyB = TRACKS.map(t => t.sky[2]).concat([TRACKS[0].sky[2]]);

    // colour table: 3 skies x 96 materials x 16 fog levels (the editor track reuses sky 1)
    consts.NMF = C.NMAT * C.NFOG;
    lists.colA = buildColourTable(mats, TRACKS[0].sky);
    lists.colB = buildColourTable(mats, TRACKS[1].sky);
    lists.colC = buildColourTable(mats, TRACKS[2].sky);
    lists.colTab = new Array(C.NMAT * C.NFOG).fill('#000000');

    // car model
    const { V, F } = carModel();
    if (V.length > C.NCARV) throw new Error('car verts ' + V.length);
    if (F.length > C.NCARF) throw new Error('car faces ' + F.length);
    consts.NCV = V.length; consts.NCF = F.length;
    lists.cvX = V.map(p => p[0]); lists.cvY = V.map(p => p[1]); lists.cvZ = V.map(p => p[2]);
    lists.cfA = F.map(f => f[0]); lists.cfB = F.map(f => f[1]); lists.cfC = F.map(f => f[2]); lists.cfD = F.map(f => f[3]); lists.cfK = F.map(f => f[4]);
    ...
    const CARS = [
        { n: 'VIPER GT', acc: 17.5, top: 88, grip: 1.00, mass: 1.00, col: 1 },
        { n: 'RALLY-X', acc: 16.0, top: 76, grip: 1.18, mass: 1.08, col: 4 },
        { n: 'BULLET S', acc: 15.0, top: 102, grip: 0.86, mass: 0.94, col: 2 },
        { n: 'DRIFTER', acc: 16.8, top: 84, grip: 0.80, mass: 1.02, col: 3 },
    ];
    ...
    for (const k of ['caX', 'caY', 'caZ', 'caYaw', 'caVX', 'caVZ', 'caVY', 'caYR', 'caSeg', 'caLap', 'caCP', 'caProg', 'caRank',
        ...
        lists[k] = zeros(NC + 1);
    for (const k of ['ccX', 'ccY', 'ccZ', 'ccSX', 'ccSY', 'ccVZ']) lists[k] = zeros(C.NCARV + 1);
    // particles
    ...
    lists.recLap = [0, 0, 0, 0]; lists.recRace = [0, 0, 0, 0];
    // sorting scratch
...
const TEXTS = [
    ...
    ['tM1', 0, -46, 14, '#ffffff', 0],
    ['tM2', 0, -62, 14, '#ffffff', 0],
    ['tM3', 0, -78, 14, '#ffffff', 0],
    ['tM4', 0, -94, 14, '#ffffff', 0],
    ['tM5', 0, -110, 14, '#ffffff', 0],
    ['tM6', 0, 126, 12, '#93a0bb', 0],
];
