...
const browser = await chromium.launch({ args: gpu ? ['--use-gl=angle', '--enable-gpu'] : [] });
...
if (opt('count', null)) {
    await page.evaluate(() => {
        ...
        window.__f0 = Number(Entry.variableContainer.variables_.find(v => v.name_ === "frameId").value_);
    });
    await page.waitForTimeout(Number(opt('count')));
    const res = await page.evaluate(() => ({ frames: Number(Entry.variableContainer.variables_.find(v => v.name_ === "frameId").value_) - window.__f0, cnt: [...window.__cnt.entries()], typ: [...window.__typ.entries()] }));
    ...
    console.log(`frames ${res.frames}, blocks/frame ${(total / res.frames).toFixed(0)}`);
